...
class Painter(torch.nn.Module):
    def __init__(
        ...
        self.thresh = (
            self.set_attention_threshold_map() if self.attention_init else None
        )
    ...
    def get_path(self):
        ...
        p0 = (
            self.inds_normalised[self.strokes_counter]
    ...
    def render_warp(self):
        if self.opacity_optim:
            for group in self.shape_groups:
                group.stroke_color.data[:3].clamp_(0.0, 0.0)  # to force black stroke
    ...
    def dino_attn(self):
        ...
        # for dino model
        mean_imagenet = torch.Tensor([0.485, 0.456, 0.406])[None, :, None, None].to(
            self.device
        )
        std_imagenet = torch.Tensor([0.229, 0.224, 0.225])[None, :, None, None].to(
            self.device
        )
    ...
    def clip_attn(self):
        ...
        if "RN" in self.saliency_clip_model:
            ...
            attn_map = interpret(
                self.image_input_attn_clip, text_input, model, device=self.device
            )
    ...
    def set_inds_clip(self):
        ...
        if self.xdog_intersec:
            xdog = XDoG_()
            im_xdog = xdog(
                self.image_input_attn_clip[0].permute(1, 2, 0).cpu().numpy(), k=10
            )
        ...
        attn_map_soft[attn_map > 0] = self.softmax(
            attn_map[attn_map > 0], tau=self.softmax_temp
        )
    ...
    def set_inds_dino(self):
        k = max(
            3, (self.num_stages * self.num_paths) // 6 + 1
        )  # sample top 3 three points from each attention head
        num_heads = self.attention_map.shape[0]
        self.inds = np.zeros((k * num_heads, 2))
        # "thresh" is used for visualisaiton purposes only
        thresh = torch.zeros(
            num_heads + 1, self.attention_map.shape[1], self.attention_map.shape[2]
        )
        ...
        for i in range(num_heads):
            ...
            inds = np.random.choice(
                range(cur_attn_map.flatten().shape[0]), size=k, replace=False, p=prob
            )
        ...
        prob_sum = sum_attn[
            self.inds[:, 0].astype(np.int), self.inds[:, 1].astype(np.int)
        ]
...
def interpret(image, texts, model, device):
    ...
    image_relevance = torch.nn.functional.interpolate(
        image_relevance, size=224, mode="bicubic"
    )
    image_relevance = (
        image_relevance.reshape(224, 224).data.cpu().numpy().astype(np.float32)
    )
